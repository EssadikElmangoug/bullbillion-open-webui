							...
							>
								...
								<div class=" flex justify-between mt-1.5 mb-2.5 mx-0.5 max-w-full">
									<div class="ml-1 self-end gap-0.5 flex items-center flex-1 max-w-[80%]">
										...
										>
											<button
												class="bg-transparent hover:bg-gray-100 text-gray-800 dark:text-white dark:hover:bg-gray-800 transition rounded-full p-1.5 outline-hidden focus:outline-hidden {userType === 'guest' ? 'cursor-not-allowed' : 'cursor-pointer'}"
												type="button"
												aria-label="More"
										...
										{#if !history?.currentId || history.messages[history.currentId]?.done == true}
											<Tooltip content={$i18n.t('Record voice')}>
												<button
													...
													class=" text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 transition rounded-full p-1.5 mr-0.5 self-center {userType === 'guest' ? 'cursor-not-allowed' : 'cursor-pointer'}"
													type="button"
													disabled={userType === 'guest'}
													on:click={async () => {
														try {
													...
													<Tooltip content={$i18n.t('Call')}>
														<button
															...
															($settings?.webSearch ?? false) === 'always'
																...
																: ''} transition rounded-full p-1.5 self-center {userType === 'guest' ? 'cursor-not-allowed bg-gray-200 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-700' : 'cursor-pointer bg-black text-white hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100'}"
															type="button"
															disabled={userType === 'guest'}
															on:click={async () => {
